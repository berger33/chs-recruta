...
def main():
    ...
    username=os.getenv("DEMO_ADMIN_USERNAME","demo").strip().lower(); password=os.getenv("DEMO_ADMIN_PASSWORD","demo12345")
    ...
    with SessionLocal() as db:
        ...
        if not user: user=User(username=username,display_name="Administrador Demonstração",email=email,password_hash=hash_password(password),platform_admin=True); db.add(user); db.flush()
